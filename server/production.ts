...
async function registerRoutes(app: Express): Promise<Server> {
  ...
  // APK management endpoints are now handled by routes.ts to avoid conflicts
  // All APK routes (/api/admin/apk/*) removed from production.ts to prevent
  // race conditions that caused file truncation in Docker environment
