...
export async function seedDatabase() {
  try {
    ...
    const createdStudios: Studio[] = [];
    for (const studioInfo of studioData) {
      const studio = await storage.createStudio(studioInfo);
      createdStudios.push(studio);
    }

    // Seed initial stream data
    const streamData: Omit<InsertStream, 'id'>[] = [
      // Featured streams
      {
        title: 'Featured Live Production',
        thumbnail: '/generated_images/Featured_live_production_15b7d8b1.png',
      ...
      // Rehearsal Room feed
      {
        ...
        url: 'webrtc://localhost:1985/live/rehearsal',
        category: 'studios',
        studioId: createdStudios[4].id
      }
    ];

    for (const streamInfo of streamData) {
      await storage.createStream(streamInfo);
    }
